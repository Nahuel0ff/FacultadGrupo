...
type
	...
	begin
		...
		readln(f.mes);			
		writeln('Ingresa el anio del final rendido: ');
		readln(f.anio);	
	end; 
	...
	procedure cargarFinales (var l:listaFinales);
	var
		f:finales;
		seguridad:integer;
	begin
		writeln('Ingrese -1 si quiere salir de agergar finales: ');
		readln(seguridad);
		leerFinal(f);
			while (seguridad <> -1) do begin
				agregarAd(l,f);
				writeln('Ingrese -1 si quiere salir de agergar finales: ');
				readln(seguridad);
				leerFinal(f);
	...
	begin
		if (a=nil) then begin
			...
			if (al.legajo < a^.dato.legajo) then agregar(a^.HI,al)
			else agregar(a^.HD,al);
	end;
	...
	begin
		if (a <> nil) then begin
			...
			writeln('Cantidad de finales ap: ', contarApro(a^.dato.nota));
			informar(a^.HD);
		end;
	end;
